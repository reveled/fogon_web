...
/**
 * Crafto widget for tours.
 *
 * @package Crafto
 */

// If class `Tour` doesn't exists yet.
if ( ! class_exists( 'CraftoAddons\Widgets\Tour' ) ) {
	/**
	 * Define `Tour` class.
	 */
	class Tour extends Widget_Base {
		/**
		 * Retrieve the list of scripts the tour widget depended on.
		 *
		...
		public function get_script_depends() {
			...
			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				$tours_scripts[] = 'crafto-widgets';
			} else {
				if ( crafto_disable_module_by_key( 'imagesloaded' ) ) {
					$tours_scripts[] = 'imagesloaded';
				}
				if ( crafto_disable_module_by_key( 'isotope' ) ) {
					$tours_scripts[] = 'isotope';
				}
				if ( crafto_disable_module_by_key( 'infinite-scroll' ) ) {
					$tours_scripts[] = 'infinite-scroll';
				}
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$tours_scripts[] = 'swiper';
					if ( '0' === $crafto_disable_all_animation && crafto_disable_module_by_key( 'anime' ) ) {
						$tours_scripts[] = 'crafto-magic-cursor';
					}
				}
				$tours_scripts[] = 'crafto-default-carousel';
				$tours_scripts[] = 'crafto-tour-widget';
			}
			return $tours_scripts;
		}

		/**
		 * Retrieve the list of styles the tour widget depended on.
		 *
		...
		public function get_style_depends() {
			...
			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				if ( is_rtl() ) {
					$tours_styles[] = 'crafto-widgets-rtl';
				} else {
					$tours_styles[] = 'crafto-widgets';
				}
			} else {
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$tours_styles[] = 'swiper';
					$tours_styles[] = 'nav-pagination';

					if ( is_rtl() ) {
						$tours_styles[] = 'nav-pagination-rtl';
					}

					if ( '0' === $crafto_disable_all_animation ) {
						$tours_styles[] = 'crafto-magic-cursor';
					}
				}

				if ( is_rtl() ) {
					$tours_styles[] = 'crafto-star-rating-rtl';
					$tours_styles[] = 'crafto-tour-rtl-widget';
				}
				$tours_styles[] = 'crafto-star-rating';
				$tours_styles[] = 'crafto-tours-widget';
			}
			return $tours_styles;
		}
		...
		public function get_title() {
			return esc_html__( 'Crafto Tour', 'crafto-addons' );
		}

		/**
		 * Get widget icon.
		 *
		 * Retrieve tour widget icon.
		 *
		 * @access public
		 *
		...
		public function get_custom_help_url() {
			return 'https://crafto.themezaa.com/documentation/tour/';
		}
		/**
		...
		protected function register_controls() {
			...
			$this->add_control(
				'crafto_tours_categories_list',
				[
					'label'       => esc_html__( 'Destinations', 'crafto-addons' ),
					'type'        => Controls_Manager::SELECT2,
					'multiple'    => true,
					'label_block' => true,
					'options'     => function_exists( 'crafto_get_categories_list' ) ? crafto_get_categories_list( 'tour-destination' ) : [], // phpcs:ignore
				]
			);
			...
			$this->add_responsive_control(
				'crafto_tours_content_box_spacer',
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .tours-packages-wrapper .tours-slider' => 'margin-bottom: {{SIZE}}{{UNIT}};',
					],
					'condition'  => [
						'crafto_tours_style' => 'tours-slider',
					],
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .price-wrap .discount-price-separator' => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_tours_show_discount_price' => 'yes',
					],
			...
			$this->add_control(
				'crafto_tours_star_reviews_color',
				[
					'label'     => esc_html__( 'Color', 'crafto-addons' ),
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
		...
		/**
		 * Render tours widget output on the frontend.
		 *
		...
		protected function render() {
			...
			$crafto_tours_post_per_page         = $this->get_settings( 'crafto_tours_post_per_page' );
			$crafto_tour_offset                 = $this->get_settings( 'crafto_tour_offset' );
			...
			$crafto_categories_list             = $this->get_settings( 'crafto_tours_categories_list' );
			...
			$query_args = array(
				...
				'post_status'    => 'publish',
				'paged'          => $paged,
				'posts_per_page' => intval( $crafto_tours_post_per_page ), // phpcs:ignore
			);

			if ( ! empty( $crafto_tour_offset ) ) {
				$query_args['offset'] = $crafto_tour_offset;
			}

			if ( ! empty( $crafto_categories_list ) ) {
