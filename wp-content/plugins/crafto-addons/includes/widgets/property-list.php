...
/**
 *
 * Crafto widget for property.
 *
 * @package Crafto
 */

// If class `Property_List` doesn't exists yet.
if ( ! class_exists( 'CraftoAddons\Widgets\Property_List' ) ) {
	/**
	 * Define `Property_List` class.
	 */
	class Property_List extends Widget_Base {
		/**
		 * Retrieve the list of scripts the property widget depended on.
		 *
		...
		public function get_script_depends() {
			$property_list_scripts = [];

			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				$property_list_scripts[] = 'crafto-widgets';
			} else {
				if ( crafto_disable_module_by_key( 'imagesloaded' ) ) {
					$property_list_scripts[] = 'imagesloaded';
				}

				if ( crafto_disable_module_by_key( 'isotope' ) ) {
					$property_list_scripts[] = 'isotope';
				}

				if ( crafto_disable_module_by_key( 'infinite-scroll' ) ) {
					$property_list_scripts[] = 'infinite-scroll';
				}
				$property_list_scripts[] = 'crafto-property-widget';
			}
			return $property_list_scripts;
		}
		/**
		 * Retrieve the list of styles the property widget depended on.
		 *
		...
		public function get_style_depends() {
			$property_list_styles   = [];

			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				if ( is_rtl() ) {
					$property_list_styles[] = 'crafto-widgets-rtl';
				} else {
					$property_list_styles[] = 'crafto-widgets';
				}
			} else {
				if ( is_rtl() ) {
					$property_list_styles[] = 'crafto-property-rtl-widget';
				}
				$property_list_styles[] = 'crafto-property-widget';
			}
			return $property_list_styles;
		}
		...
		public function get_title() {
			return esc_html__( 'Crafto Property List', 'crafto-addons' );
		}
		...
		public function get_custom_help_url() {
			return 'https://crafto.themezaa.com/documentation/property-list/';
		}
		/**
		...
		/**
		 * Register property list widget controls.
		 *
		...
		protected function register_controls() {
			...
			$this->add_control(
				...
				[
					...
					'multiple'    => true,
					'label_block' => true,
					'options'     => function_exists( 'crafto_get_tags_list' ) ? crafto_get_tags_list( 'properties-agents' ) : [], // phpcs:ignore
					'condition'   => [
			...
			$crafto_property_status_default = array(
			...
			$crafto_property_status = apply_filters( 'crafto_property_status_list', $crafto_property_status );
			$crafto_property_status = $crafto_property_status ? $crafto_property_status : [];
			...
			$this->add_control(
				...
				[
					...
					'type'        => Controls_Manager::SELECT,
					'label_block' => true,
					'options'     => array_merge( $crafto_property_status_default, $crafto_property_status ), // phpcs:ignore
					'default'     => 'default',
					'condition'   => [
			...
			$this->add_group_control(
				...
				[
					'name'     => 'crafto_property_content_boxshadow',
					'selector' => '{{WRAPPER}} .property-wrapper .property-details-content-wrap',
				]
			);
			$this->add_group_control(
			...
			$this->add_responsive_control(
				'crafto_property_content_border_radius',
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .property-wrapper .property-details-content-wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
			);
			$this->add_responsive_control(
				'crafto_property_content_padding',
				[
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .property-wrapper .properties-details .content-wrapper' => 'border-color: {{VALUE}};',
					],
				]
			);
			$this->add_responsive_control(
				...
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .property-wrapper .property-action-wrap' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
					'condition'  => [
						'crafto_pagination' => 'load-more-pagination',
					],
			...
			$this->add_control(
				'crafto_property_button_hover_border',
				[
					'label'     => esc_html__( 'Border Color', 'crafto-addons' ),
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
			...
			$this->add_control(
				'crafto_property_button_separator',
				[
			...
			$this->add_responsive_control(
				'crafto_property_button_padding',
				[
					'label'      => esc_html__( 'Padding', 'crafto-addons' ),
					'type'       => Controls_Manager::DIMENSIONS,
					'size_units' => [
					...
					'selectors'  => [
						'{{WRAPPER}} .elementor-button-wrapper .property-post-button' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
			);
			$this->end_controls_section();
		...
		protected function render() {
			...
			$property_post_per_page            = ( isset( $settings['crafto_property_per_page'] ) && $settings['crafto_property_per_page'] ) ? $settings['crafto_property_per_page'] : 6;
			$crafto_property_offset            = ( isset( $settings['crafto_property_offset'] ) && $settings['crafto_property_offset'] ) ? $settings['crafto_property_offset'] : '';
			...
			$query_args = array(
				...
				'post_status'    => 'publish',
				'posts_per_page' => intval( $property_post_per_page ), // phpcs:ignore
				'paged'          => $paged,
			);

			if ( ! empty( $crafto_property_offset ) ) {
				$query_args['offset'] = $crafto_property_offset;
			}
