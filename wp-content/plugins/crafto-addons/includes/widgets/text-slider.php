...
/**
 * Crafto widget for text slider.
 *
 * @package Crafto
 */

// If class `Text_Slider` doesn't exists yet.
if ( ! class_exists( 'CraftoAddons\Widgets\Text_Slider' ) ) {
	/**
	 * Define `Text_Slider` class.
	 */
	class Text_Slider extends Widget_Base {
		/**
		 * Retrieve the list of scripts the text slider widget depended on.
		 *
		...
		public function get_script_depends() {
			...
			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				$text_slider_scripts[] = 'crafto-widgets';
			} else {
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$text_slider_scripts[] = 'swiper';
					if ( '0' === $crafto_disable_all_animation && crafto_disable_module_by_key( 'anime' ) ) {
						$text_slider_scripts[] = 'crafto-magic-cursor';
					}
				}
				$text_slider_scripts[] = 'crafto-text-slider-widget';
			}

			return $text_slider_scripts;
		}

		/**
		 * Retrieve the list of styles the text slider widget depended on.
		 *
		...
		public function get_style_depends() {
			...
			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				if ( is_rtl() ) {
					$text_slider_styles[] = 'crafto-widgets-rtl';
				} else {
					$text_slider_styles[] = 'crafto-widgets';
				}
			} else {
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$text_slider_styles[] = 'swiper';
					$text_slider_styles[] = 'nav-pagination';

					if ( is_rtl() ) {
						$text_slider_styles[] = 'nav-pagination-rtl';
					}

					if ( '0' === $crafto_disable_all_animation ) {
						$text_slider_styles[] = 'crafto-magic-cursor';
					}
				}

				if ( is_rtl() ) {
					$text_slider_styles[] = 'crafto-text-slider-rtl';
				}
				$text_slider_styles[] = 'crafto-text-slider-widget';
			}
			return $text_slider_styles;
		}
		...
		public function get_title() {
			return esc_html__( 'Crafto Text Slider', 'crafto-addons' );
		}
		...
		public function get_custom_help_url() {
			return 'https://crafto.themezaa.com/documentation/text-slider/';
		}
		/**
		...
		/**
		 * Register text slider widget controls.
		 *
		...
		protected function register_controls() {
			...
			$this->add_control(
				...
				[
					...
					'default' => 'text-slider-style-1',
					'options' => [
						'text-slider-style-1' => esc_html__( 'Style 1', 'crafto-addons' ),
						'text-slider-style-2' => esc_html__( 'Style 2', 'crafto-addons' ),
					],
				]
			);
			...
			$repeater->add_control(
				...
				[
					...
					'description' => esc_html__( 'Applicable in style 1 only.', 'crafto-addons' ),
					'label_block' => true,
				]
			);
			$repeater->add_control(
			...
			$this->add_responsive_control(
				...
				[
					...
					'condition' => [
						'crafto_carousel_slide_styles' => 'text-slider-style-1',
					],
				]
			);
			...
			$this->add_group_control(
				Group_Control_Background::get_type(),
				[
					'name'     => 'crafto_feature_box_subtitle_background',
					'exclude'  => [
						'image',
						'position',
						'attachment',
			...
			$this->add_control(
				'crafto_feature_box_subtitle_radius',
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .swiper-slide .subtitle' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
			);
			$this->add_responsive_control(
				'crafto_feature_box_subtitle_padding',
				[
					'label'      => esc_html__( 'Padding', 'crafto-addons' ),
					'type'       => Controls_Manager::DIMENSIONS,
					'size_units' => [
					...
					'selectors'  => [
						'{{WRAPPER}} .swiper-slide .subtitle' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
				]
			);
			$this->end_controls_section();
		...
		/**
		 * Render Text Slider widget output on the frontend.
		 *
		...
		protected function render() {
			...
			switch ( $slide_styles ) {
				case 'text-slider-style-1':
					foreach ( $settings['crafto_carousel_slider'] as $index => $item ) {
						...
						if ( ! empty( $item['crafto_carousel_title'] ) || ! empty( $item['crafto_carousel_subtitle'] ) || ! empty( $item['crafto_content'] ) ) {
							?>
							<div <?php $this->print_render_attribute_string( $wrapper_key ); ?>>
								<div class="swiper-slider">
									...
									if ( ! empty( $item['crafto_carousel_subtitle'] ) ) {
										?>
										<div class="subtitle">
											<?php echo esc_html( $item['crafto_carousel_subtitle'] ); // phpcs:ignore ?>
										</div>
										<?php
									}
									...
									if ( ! empty( $item['crafto_carousel_title'] ) ) {
										?>
										<<?php echo $this->get_settings( 'crafto_text_slider_title_size' ); // phpcs:ignore ?>  class="title">
											<?php echo esc_html( $item['crafto_carousel_title'] ); // phpcs:ignore ?>
										</<?php echo $this->get_settings( 'crafto_text_slider_title_size' ); // phpcs:ignore ?>>
										<?php
									}
				...
				case 'text-slider-style-2':
					foreach ( $settings['crafto_carousel_slider'] as $index => $item ) {
						...
						if ( ! empty( $item['crafto_carousel_title'] ) || ! empty( $item['crafto_content'] ) ) {
							?>
							<div <?php $this->print_render_attribute_string( $wrapper_key ); ?>>
								<div class="swiper-slider">
			...
			$sliderconfig = array(
				...
				'reversedirection' => $this->get_settings( 'crafto_reversedirection' ),
