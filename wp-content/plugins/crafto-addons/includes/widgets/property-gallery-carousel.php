...
/**
 *
 * Crafto widget for property gallery carousel.
 *
 * @package Crafto
 */
...
// If class `Property_Gallery_Carousel` doesn't exists yet.
if ( ! class_exists( 'CraftoAddons\Widgets\Property_Gallery_Carousel' ) ) {
	/**
	 * Define `Property_Gallery_Carousel` class.
	 */
	class Property_Gallery_Carousel extends Widget_Base {
		/**
		 * Retrieve the list of scripts the property gallery carousel widget depended on.
		 *
		...
		public function get_script_depends() {
			$propert_gallery_carousel_scripts = [];
			$crafto_disable_all_animation     = get_theme_mod( 'crafto_disable_all_animation', '0' );

			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				$propert_gallery_carousel_scripts[] = 'crafto-widgets';
			} else {
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$propert_gallery_carousel_scripts[] = 'swiper';

					if ( '0' === $crafto_disable_all_animation && crafto_disable_module_by_key( 'anime' ) ) {
						$propert_gallery_carousel_scripts[] = 'crafto-magic-cursor';
					}
				}
				$propert_gallery_carousel_scripts[] = 'crafto-default-carousel';
			}
			return $propert_gallery_carousel_scripts;
		}
		/**
		 * Retrieve the list of styles the property gallery carousel widget depended on.
		 *
		...
		public function get_style_depends() {
			$propert_gallery_carousel_styles = [];

			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				$propert_gallery_carousel_styles[] = 'crafto-widgets';
			} else {
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$crafto_disable_all_animation = get_theme_mod( 'crafto_disable_all_animation', '0' );

					$propert_gallery_carousel_styles[] = 'swiper';
					$propert_gallery_carousel_styles[] = 'nav-pagination';

					if ( is_rtl() ) {
						$propert_gallery_carousel_styles[] = 'nav-pagination-rtl';
					}

					if ( '0' === $crafto_disable_all_animation ) {
						$propert_gallery_carousel_styles[] = 'crafto-magic-cursor';
					}
				}
				$propert_gallery_carousel_styles[] = 'crafto-property-gallery-carousel';
			}
			return $propert_gallery_carousel_styles;
		}
		...
		public function get_name() {
			return 'crafto-property-gallery-carousel';
		}
		...
		public function get_title() {
			return esc_html__( 'Crafto Property Gallery Carousel', 'crafto-addons' );
		}
		...
		public function get_custom_help_url() {
			return 'https://crafto.themezaa.com/documentation/property-gallery-carousel/';
		}
		/**
		...
		/**
		 * Register property gallery carousel widget controls.
		 *
		...
		protected function register_controls() {
			$this->start_controls_section(
				'crafto_gallery_content_section',
				[
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf( esc_html__( 'These are placeholder images, the original carousel images are pulled from relavent property post.', 'crafto-addons' ) ),
					'separator'       => 'after',
		...
		/**
		 * Render icon property gallery carousel widget output on the frontend.
		 *
		...
		protected function render() {
			...
			$crafto_gallery_images          = crafto_post_meta( 'crafto_property_multiple_image' );
			...
			if ( $crafto_gallery_images ) {
				$crafto_gallery = explode( ',', $crafto_gallery_images );
			}

			$placeholder_array = array_fill( 0, 6, Utils::get_placeholder_image_src() );
			foreach ( $placeholder_array as $url_val ) {
				$crafto_demo_gallery[]['url'] = $url_val;
			}

			if ( ( is_array( $crafto_gallery ) && ! empty( $crafto_gallery ) ) || ! empty( $crafto_demo_gallery ) ) {
				...
				if ( ! empty( $crafto_gallery ) ) {
					foreach ( $crafto_gallery as $key => $value ) {
						...
						?>
						<div <?php $this->print_render_attribute_string( 'slide-wrap' ); ?>>
							<?php echo wp_get_attachment_image( $value, $crafto_thumbnail, '', $crafto_img_attr ); ?>
						</div>
						<?php
					}
				} else {
					foreach ( $crafto_demo_gallery as $key => $value ) {
						$image_url = isset( $value['url'] ) ? $value['url'] : '';
						?>
						<div <?php $this->print_render_attribute_string( 'slide-wrap' ); ?>>
						<?php
						if ( ! empty( $image_url ) ) {
							echo '<img src="' . esc_attr( $image_url ) . '" alt="' . esc_attr__( 'Property Gallery Image', 'crafto-addons' ) . '">';
						}
						?>
						</div>
						<?php
				...
				$slides_count = count( $crafto_gallery ) > 0 ? count( $crafto_gallery ) : count( $crafto_demo_gallery );
