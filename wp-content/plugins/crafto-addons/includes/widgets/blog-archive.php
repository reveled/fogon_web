...
/**
 *
 * Crafto widget for blog archive.
 *
 * @package Crafto
 */
if ( \Elementor\Plugin::instance()->editor->is_edit_mode() && class_exists( 'Crafto_Builder_Helper' ) && ! \Crafto_Builder_Helper::is_theme_builder_archive_template() ) {
	return;
}

// If class `Blog_Archive` doesn't exists yet.
if ( ! class_exists( 'CraftoAddons\Widgets\Blog_Archive' ) ) {
	/**
	 * Define `Blog_Archive` class.
	 */
	class Blog_Archive extends Widget_Base {
		...
		public function get_title() {
			return esc_html__( 'Crafto Blog Archive', 'crafto-addons' );
		}
		...
		public function get_icon() {
			return 'eicon-archive crafto-element-icon';
		}
		...
		public function get_custom_help_url() {
			return 'https://crafto.themezaa.com/documentation/blog-archive/';
		}
		...
		/**
		 * Retrieve the list of scripts the blog archive widget depended on.
		 *
		...
		public function get_script_depends() {
			$blog_archive_scripts = [];

			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				$blog_archive_scripts[] = 'crafto-widgets';
			} else {
				if ( crafto_disable_module_by_key( 'imagesloaded' ) ) {
					$blog_archive_scripts[] = 'imagesloaded';
				}

				if ( crafto_disable_module_by_key( 'isotope' ) ) {
					$blog_archive_scripts[] = 'isotope';
				}

				if ( crafto_disable_module_by_key( 'infinite-scroll' ) ) {
					$blog_archive_scripts[] = 'infinite-scroll';
				}

				if ( crafto_disable_module_by_key( 'fitvids' ) ) {
					$blog_archive_scripts[] = 'jquery.fitvids';
				}

				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$blog_archive_scripts[] = 'swiper';
				}

				if ( crafto_disable_module_by_key( 'magnific-popup' ) ) {
					$blog_archive_scripts[] = 'magnific-popup';
					$blog_archive_scripts[] = 'crafto-lightbox-gallery';
				}
				$blog_archive_scripts[] = 'crafto-blog-list-widget';
			}
			return $blog_archive_scripts;
		}

		/**
		 * Retrieve the list of styles the blog archive widget depended on.
		 *
		...
		public function get_style_depends() {
			...
			if ( \Crafto_Addons_Extra_Functions::crafto_editor_load_compressed_assets() ) {
				if ( is_rtl() ) {
					$blog_archive_styles[] = 'crafto-widgets-rtl';
				} else {
					$blog_archive_styles[] = 'crafto-widgets';
				}
			} else {
				if ( crafto_disable_module_by_key( 'swiper' ) ) {
					$blog_archive_styles[] = 'swiper';
				}

				if ( is_rtl() ) {
					$blog_archive_styles[] = 'crafto-blog-list-rtl-widget';
				}
				$blog_archive_styles[] = 'crafto-blog-list-widget';
			}
			return $blog_archive_styles;
		}
		...
		protected function register_controls() {
			...
			$this->add_control(
				'crafto_exclude_post',
				[
					'label'       => __( 'Exclude Posts', 'crafto-addons' ),
					'type'        => Controls_Manager::SELECT2,
					'multiple'    => true,
					'label_block' => true,
					'options'     => $this->crafto_get_post_array(),
					'description' => esc_html__( 'You can use this option to remove certain posts, pages or custom post types from the list.', 'crafto-addons' ),
				]
			);
			$this->end_controls_section();
			$this->start_controls_section(
			...
			$this->add_group_control(
				...
				[
					...
					'selector' => '{{WRAPPER}} .crafto-blog-list .post-date',
				]
			);
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .post-date, {{WRAPPER}} .blog-masonry .post-date-wrapper' => 'color: {{VALUE}};',
					],
				]
			);
			$this->add_control(
			...
			$this->add_group_control(
				...
				[
					...
					'selector'       => '{{WRAPPER}} .crafto-blog-list .author-name, {{WRAPPER}} .crafto-blog-list .author-name a',
					'fields_options' => [
						'typography' => [
			...
			$this->start_controls_tabs( 'crafto_blog_list_meta_author_tabs' );
				...
				$this->add_group_control(
					...
					[
						...
						'selector'       => '{{WRAPPER}} .crafto-blog-list .author-name',
						'fields_options' => [
							'background' => [
				...
				$this->add_group_control(
					...
					[
						...
						'selector'       => '{{WRAPPER}} .crafto-blog-list .author-name a',
						'fields_options' => [
							'background' => [
				...
				$this->add_control(
					...
					[
						...
						'type'      => Controls_Manager::COLOR,
						'selectors' => [
							'{{WRAPPER}} .crafto-blog-list .author-name i' => 'color: {{VALUE}};',
						],
						'condition' => [
							'crafto_blog_style' => [
								'blog-standard',
				...
				$this->add_group_control(
					...
					[
						...
						'selector'       => '{{WRAPPER}} .crafto-blog-list .author-name a:hover, {{WRAPPER}} .blog-only-text .blog-post:hover .author-name a',
						'fields_options' => [
							'background' => [
								'label' => esc_html__( 'Author Color', 'crafto-addons' ),
							],
				...
				$this->add_control(
					...
					[
						...
						'type'      => Controls_Manager::COLOR,
						'selectors' => [
							'{{WRAPPER}} .crafto-blog-list .author-name a:hover' => 'border-color: {{VALUE}};',
						],
						'condition' => [
							'crafto_blog_style' => [
								'blog-grid',
			...
			$this->add_group_control(
				...
				[
					...
					'selector'  => '{{WRAPPER}} .crafto-blog-list .post-meta-like a',
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
							'blog-full-image',
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .post-meta-like a i' => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .posts-like-count' => 'color: {{VALUE}};',
						'{{WRAPPER}} .crafto-blog-list .posts-like'       => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .post-meta-like a:hover .posts-like-count' => 'color: {{VALUE}};',
						'{{WRAPPER}} .crafto-blog-list .post-meta-like a:hover .posts-like' => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
			...
			$this->add_group_control(
				...
				[
					...
					'selector'  => '{{WRAPPER}} .crafto-blog-list .post-meta-comments a',
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
							'blog-full-image',
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .post-meta-comments a i' => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .comment-count' => 'color: {{VALUE}};',
						'{{WRAPPER}} .crafto-blog-list .comment-text'  => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
			...
			$this->add_control(
				...
				[
					...
					'type'      => Controls_Manager::COLOR,
					'selectors' => [
						'{{WRAPPER}} .crafto-blog-list .post-meta-comments a:hover .comment-count' => 'color: {{VALUE}};',
						'{{WRAPPER}} .crafto-blog-list .post-meta-comments a:hover .comment-text'  => 'color: {{VALUE}};',
					],
					'condition' => [
						'crafto_blog_style!' => [
							'blog-metro',
			...
			$this->start_controls_tabs( 'crafto_blog_list_meta_categories_tabs' );
				...
				$this->add_group_control(
					...
					[
						...
						'selector'  => '{{WRAPPER}} .crafto-blog-list .blog-category a:not(.post-date)',
						'condition' => [
							'crafto_blog_style!' => [
								'blog-side-image',
								'hero-blog',
				...
				$this->add_responsive_control(
					...
					[
						...
						'selectors'  => [
							'{{WRAPPER}} .crafto-blog-list .blog-category a:not(.post-date)' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
						],
						'condition'  => [
							'crafto_blog_style!' => [
								'blog-side-image',
				...
				$this->add_responsive_control(
					...
					[
						...
						'selectors'  => [
							'{{WRAPPER}} .crafto-blog-list .blog-category a' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
						],
						'condition'  => [
							'crafto_blog_style!' => [
								'blog-simple',
			...
			$this->add_group_control(
				...
				[
					...
					'selector'       => '{{WRAPPER}} .horizontal-separator, {{WRAPPER}} .crafto-blog-list .post-meta-separator',
					'condition'      => [
						'crafto_blog_style' => [
							'hero-blog',
							'blog-modern',
			...
			$this->add_control(
				...
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .horizontal-separator' => 'height: {{SIZE}}{{UNIT}};',
						'{{WRAPPER}} .crafto-blog-list .post-meta-separator' => 'width: {{SIZE}}{{UNIT}};',
					],
					'condition'  => [
						'crafto_blog_style' => [
							'hero-blog',
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .horizontal-separator' => 'width: {{SIZE}}{{UNIT}};',
						'{{WRAPPER}} .crafto-blog-list .post-meta-separator' => 'height: {{SIZE}}{{UNIT}};',
					],
					'condition'  => [
						'crafto_blog_style' => [
							'hero-blog',
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors'  => [
						'{{WRAPPER}} .crafto-blog-list .post-meta-separator, {{WRAPPER}} .horizontal-separator' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					],
					'condition'  => [
						'crafto_blog_style' => [
							'hero-blog',
		...
		/**
		 * Render archive posts widget output on the frontend.
		 *
		...
		protected function render() {
			...
			$crafto_exclude_post        = $this->get_settings( 'crafto_exclude_post' );
			...
			$crafto_archive_offset      = $this->get_settings( 'crafto_archive_offset' );
			...
			$crafto_category_limit                  = ( isset( $settings['crafto_category_limit'] ) && $settings['crafto_category_limit'] ) ? $settings['crafto_category_limit'] : '';
			...
			if ( $paged < 2 && ! is_singular( 'themebuilder' ) ) {
				while ( have_posts() ) :
					the_post();
					...
					if ( is_sticky() && is_home() ) {
						...
						$post_separator            = '<span class="post-meta-separator">•</span>';
			...
			if ( Plugin::$instance->editor->is_edit_mode() || Plugin::$instance->preview->is_preview_mode() ) {
				$query_args['post_type'] = 'post'; //phpcs:ignore
				...
				$post_type = ( isset( $_GET['post_type'] ) && $_GET['post_type'] ) ? $_GET['post_type'] : array( 'post', 'page' ); //phpcs:ignore
			...
			if ( ! empty( $crafto_archive_offset ) ) {
				$query_args['offset'] = $crafto_archive_offset;
			}

			if ( 'yes' === $crafto_ignore_sticky_posts ) {
				$query_args['ignore_sticky_posts'] = true;
				if ( ! empty( $crafto_exclude_post ) ) {
					$crafto_exclude_post = array_merge( $crafto_exclude_post, get_option( 'sticky_posts' ) );
				} else {
			...
			if ( ! empty( $crafto_exclude_post ) ) {
				$query_args['post__not_in'] = $crafto_exclude_post;
			}
			...
			if ( $wp_query->have_posts() ) {
				...
				<ul <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
					...
					while ( $wp_query->have_posts() ) {
						...
						switch ( $crafto_blog_style ) {
								...
								if ( 'yes' === $crafto_show_post_category || 'yes' === $crafto_show_post_date || 'yes' === $crafto_show_post_title || 'yes' === $crafto_show_post_excerpt || 'yes' === $crafto_show_post_read_more_button || 'yes' === $crafto_show_post_author || 'yes' === $crafto_show_post_category || 'yes' === $crafto_show_post_like || 'yes' === $crafto_show_post_comments && ( comments_open() || get_comments_number() ) ) {
									...
									<li <?php $this->print_render_attribute_string( $inner_wrapper_key ); ?>>
										<div <?php $this->print_render_attribute_string( $blog_post_key ); ?>>
											...
											$featured_bg_url  = ( $featured_img_url ) ? ' style="background-image:url(' . esc_url( $featured_img_url ) . ');"' : '';
											if ( ! post_password_required() && ! empty( $featured_img_url ) ) {
												?>
												<div class="blog-post-images">
													<div class="post-image-wrap"<?php echo sprintf( '%s', $featured_bg_url ); // phpcs:ignore ?>></div>
												</div>
												<?php
											}
							...
							case 'blog-standard':
									...
									<li <?php $this->print_render_attribute_string( $inner_wrapper_key ); ?>>
										<div <?php $this->print_render_attribute_string( $blog_post_key ); ?>>
											...
											if ( ! post_password_required() ) {
												if ( 'yes' === $crafto_show_post_thumbnail ) {
													?>
													<div class="blog-post-images">
														<?php
														if ( 'yes' === $crafto_show_post_category ) {
								...
								break;
							case 'blog-only-text':
								if ( 'yes' === $crafto_show_post_category || 'yes' === $crafto_show_post_title || 'yes' === $crafto_show_post_excerpt || 'yes' === $crafto_show_post_read_more_button || 'yes' === $crafto_show_post_thumbnail ) {
									?>
									<li <?php $this->print_render_attribute_string( $inner_wrapper_key ); ?>>
										<div <?php $this->print_render_attribute_string( $blog_post_key ); ?>>
											<div class="box-overlay"></div>
											...
											if ( 'yes' === $crafto_show_post_count || 'yes' === $crafto_show_post_category || 'yes' === $crafto_show_post_date || 'yes' === $crafto_show_post_title || 'yes' === $crafto_show_post_excerpt ) {
												?>
												<div class="post-details">
													<?php
							...
							case 'blog-split':
								...
								$this->add_render_attribute(
									...
									[
										'class' =>
											[
												'd-sm-flex d-block align-items-center',
											],
									],
								);
					...
					get_next_posts_page_link( $wp_query->max_num_pages );
					?>
				</ul>
				<?php
				...
				<div class="alert alert-warning" role="alert">
					<strong><?php echo esc_html__( 'It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', 'crafto-addons' ); ?></strong>
				</div>
				<?php
			}
		}
		 ...
		 * @param string $textcolor Get color string.
		 * @param string $count Get counter number.
		 */
		public function crafto_post_category( $id, $textcolor = '', $count = '1' ) {
			if ( '' === $id ) {
				return;
			}
			$post_cat         = array();
			...
			$textcolor        = ( $textcolor ) ? 'class="' . esc_attr( $textcolor ) . '"' : '';
			...
			foreach ( $categories as $category ) {
				...
				$post_cat[]    = '<a rel="category tag" href="' . esc_url( $category_link ) . '">' . esc_html( $category->name ) . '</a>';
				++$category_counter;
			}
			$post_category = ( is_array( $post_cat ) && ! empty( $post_cat ) ) ? implode( ', ', $post_cat ) : '';
									...
									<a href="<?php echo esc_url( $crafto_thumb ); ?>" data-elementor-open-lightbox="no" data-group="lightbox-gallery-<?php echo esc_attr( $crafto_popup_id ); ?>" class="lightbox-group-gallery-item"<?php echo sprintf( '%s', $crafto_attachment_attributes ); //phpcs:ignore ?>>
										<figure>
											<div class="portfolio-img">
												<?php
												echo wp_get_attachment_image( $value, $crafto_srcset );
												?>
											</div>
											<figcaption>
		...
		public function crafto_get_post_array() {
			...
			$ids = get_posts([
				'post_type'              => apply_filters( 'crafto_exclude_post_type_list', $post_types ), // phpcs:ignore
				'post_status'            => 'publish',
				'posts_per_page'         => -1,
				'orderby'                => 'title',
				'order'                  => 'ASC',
			...
			if ( ! empty( $ids ) ) {
				foreach ( $ids as $id ) {
					...
					$post_array[ $id ] = $title ? $title : esc_html__( '(no title)', 'crafto-addons' );
				}
			}
			return $post_array;
		}
