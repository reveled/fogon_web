( function( $ ) {
	...
	let CraftoAddonsInit = {
		init: function init() {
			elementorFrontend.hooks.addAction( 'frontend/element_ready/crafto-interactive-portfolio.default', CraftoAddonsInit.interactivePortfolioInit );
		},
		interactivePortfolioInit: function interactivePortfolioInit( $scope ) {
 
			const container = document.querySelector( '.interactive-wrapper' );
						...
						listen( events, grabListener, isTouch ) {
							...
							if ( Array.isArray( events ) ) {
								...
								window.addEventListener(events, function (event) {
									if ( event.target.closest( '.interactive-wrapper' ) === container ) {
										listener( event );
									} else {
										_this.onMouseUp()
									}
