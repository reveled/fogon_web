	...
	e( window ).on( 'elementor:init', ( function() {
		var t = elementor.modules.controls.BaseData.extend({
			...
			addControlSpinner: function() {
				const spinnerHTML  = '';
					spinnerHTML +='<span class="elementor-control-spinner">';
					spinnerHTML +='&nbsp;<i class="eicon-spinner eicon-animation-spin"></i>&nbsp;'
					spinnerHTML +='</span>';

				this.ui.select.prop( 'disabled', !0), this.$el.find( '.elementor-control-title' ).after( spinnerHTML );
			},
			prepareArgs: function() {
				var e = this,
					t = e.model.get( 'query_args' );
				return _.isObject(t) || (t = {}), t.widget_props && _.isObject(t.widget_props) && _.each(t.widget_props, (function(n, s) {
					t[s] = e.container.settings.get(n)
				})), t
			},
			...
			getValueTitles: function() {
				...
				if (n) {
					_.isArray(n) || (n = [n]);
					var s = {
							action: crafto_dynamic_select.action,
							security: crafto_dynamic_select.nonce,
							ids: n
						},
						i = e.extend({}, s, t.getQueryData());
					e.ajax({
						url: ajaxurl,
						type: "POST",
						data: i,
						...
						success: function(e) {
							if ( t.isTitlesReceived = !0, e.success ) {
								...
								_.each(e.data, (function(e) {
									n[e.id] = e.text
								})), t.model.set( 'options', n ), t.ui.select && t.ui.select.data( 'select2' ) && t.render()
				...
				this.ui.select.select2({
					...
					ajax: {
						...
						data: function( n ) {
							...
							return e.extend({}, s, t.getQueryData())
						},
		...
		elementor.addControlView( 'crafto-dynamic-select', t )
	}))
}(jQuery);