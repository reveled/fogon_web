...
// If class `Simple_Navigation_Frontend_Walker` doesn't exists yet.
if ( ! class_exists( 'CraftoAddons\Classes\Simple_Navigation_Frontend_Walker' ) ) {

	/**
	 * Define Simple_Navigation_Frontend_Walker class
	 */
	class Simple_Navigation_Frontend_Walker extends \Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			$description = apply_filters( 'add_description_to_menu', $item->description, $item->ID );
			...
			$aria_label_attr = ' aria-label= "' . esc_html( $title ) . '"';

			$item_output .= '<a' . $attributes . " " . $aria_label_attr . ' itemprop="url" ' . $link_class_name . '>'; // phpcs:ignore

			if ( ! empty( $crafto_menu_item_icon ) && '' !== $crafto_menu_item_icon ) {
				$item_output .= '<i class="menu-item-icon ' . esc_attr( $crafto_menu_item_icon ) . '"' . $crafto_icon_color_css . '></i>';
