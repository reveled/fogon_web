...
class FormController extends FrontendRESTController
{
	...
	public function save_form_data($request)
	{
						...
						if ($field_config) {
							...
							if (!empty($field_config['allowed_types']) && !in_array($file_type, $field_config['allowed_types'], true)) {
								$form_data[$name] = new WP_Error(
									'invalid_file_type',
									sprintf(__('File type "%s" is not allowed for this field.', 'your-text-domain'), $file_type)
								);
								continue;
							}
							...
							if (!empty($field_config['max_size']) && $file_size > $field_config['max_size']) {
								$form_data[$name] = new WP_Error(
									'file_too_large',
									sprintf(__('File size exceeds the maximum allowed for this field (%s bytes).', 'your-text-domain'), $field_config['max_size'])
								);
								continue;
							}
						}
		...
		do_action( 'droip_form_submitted', $form_data, $form_config );
