...
class CompLibFormHandler extends WP_REST_Controller
{
  ...
  function handle_post_comment($request)
  {
    ...
    $transiet_name = $this->validate_nonce('droip-comment');
  ...
  {
    $form_data = $request->get_body_params();
    $transiet_name = $this->validate_nonce('droip-login');
  ...
  function handle_register($request)
  {
    ...
    $form_data = $request->get_body_params();
    $transiet_name = $this->validate_nonce('droip-register');
  ...
  {
    $form_data = $request->get_body_params();
    $transiet_name = $this->validate_nonce('droip-forgot-password');
  ...
  {
    $form_data = $request->get_body_params();
    $transiet_name = $this->validate_nonce('droip-change-password');
  ...
  {
    $form_data = $request->get_body_params();
    $transiet_name = $this->validate_nonce('droip-retrieve-username');
  ...
  function validate_nonce($element_name)
  {
    $nonce = isset($_SERVER['HTTP_X_WP_ELEMENT_NONCE'])
        ? sanitize_text_field($_SERVER['HTTP_X_WP_ELEMENT_NONCE'])
        : null;
    ...
    $action = DROIP_COMPONENT_LIBRARY_APP_PREFIX . '_' . $element_name;
