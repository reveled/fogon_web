...
class Fields {
	...
	private function setArgument( $key, $attr ) {
		...
		if ( isset( $attr['multiple'] ) && $attr['multiple'] ) {
			...
			$this->source_name = $attr['source_name'] ?? '';
		}
	...
	public function Field( $key, $attr = [] ) {
		...
		$holderClass = ! empty( $this->holderClass ) ? explode( ' ', $this->holderClass ) : [];
		$html        = null;
		...
		if ( 'title' === $this->type ) {
			return sprintf( '<div class="rt-field-wrapper rtfm-field-title"><h3 class="title-label">%s</h3></div>', $this->label );
		}
	...
	private function select() {
		...
		if ( $this->multiple ) {
			...
			$this->attr  = $this->attr . " data-post-type='" . $this->post_type . "'";
			$this->attr  = $this->attr . " data-source-name='" . $this->source_name . "'";
	...
	private function switchField() {
		$h       = null;
		$checked = $this->value ? 'checked' : null;
		$h      .= '<label class="rtfm-switch"><input type="checkbox" ' . esc_attr( $checked ) . ' id="' . esc_attr( $this->name ) . '" name="' . esc_attr( $this->name ) . '" value="on" /><span class="rtfm-switch-slider round"></span></label>';
		return $h;
	}
