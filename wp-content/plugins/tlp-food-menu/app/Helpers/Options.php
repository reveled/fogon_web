...
class Options {
	...
	public static function generalSettings() {
		...
		$general = [
			'fm_food_menu_type' => [
				...
				'type'        => 'radio',
				'alignment'   => 'vertical',
				'options'     => self::food_menu_types(),
				'value'       => ! empty( $settings['fm_food_menu_type'] ) ? $settings['fm_food_menu_type'] : 'food_menu_post',
				'holderClass' => ! TLPFoodMenu()->has_pro() ? 'rt-pro-field' : '',
				'default'     => 'food_menu_post',
				'description' => sprintf(
					'%s <br>%s',
					esc_html__( 'The food menu custom post type can be sourced from the food menu post but it does not include an online ordering system.', 'tlp-food-menu' ),
					esc_html__( 'Online ordering functionality is sourced from WooCommerce products, with WooCommerce being mandatory. All online ordering features, such as product addons, tipping, pickup/delivery options, discount specials, and special menu items, are available in the pro version.', 'tlp-food-menu' )
				),
			],
			'slug'              => [
		...
		if ( 'online_ordering' === $food_menu_type ) {
			unset( $general['currency'], $general['currency_position'] );
		}
	 ...
	 * @return array
	 */
	public static function food_menu_types() {
		return [
			'food_menu_post'  => esc_html__( 'Food Menu Post ( Custom Post )', 'tlp-food-menu' ),
			'online_ordering' => esc_html__( 'Online Ordering ( Woocommerce Product )', 'tlp-food-menu' ),
		];
	}
