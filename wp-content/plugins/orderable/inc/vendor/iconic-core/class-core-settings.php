...
if ( class_exists( 'Orderable_Core_Settings' ) ) {
	return;
}

/**
 * Orderable_Core_Settings.
 *
 * @class    Orderable_Core_Settings
 * @version  1.0.6
 */
class Orderable_Core_Settings {
	/**
	 * Single instance of the Orderable_Core_Settings object.
	 *
	 * @var Orderable_Core_Settings
	 */
	 ...
	 *
	 * @var Orderable_Settings_Framework
	 */
	...
	/**
	 * Creates/returns the single instance Orderable_Core_Settings object.
	 *
	 * @param array $args Arguments.
	 *
	 * @return Orderable_Core_Settings
	 */
	...
	private function __construct() {
		...
		if ( defined( 'ORDERABLE_FILE' ) ) {
			add_filter( 'plugin_action_links_' . plugin_basename( ORDERABLE_FILE ), array( __CLASS__, 'plugin_action_links' ) );
		}
	}
	...
	public static function init() {
		...
		self::$settings_framework = new Orderable_Settings_Framework( self::$args['settings_path'], self::$args['option_group'] );
	...
	public static function get_plugin_slug() {
		return str_replace( '-', '_', 'orderable' );
	}
	...
	public static function add_settings_page() {
		if ( ! defined( 'ICONIC_DISABLE_BRAND' ) && ! defined( 'ORDERABLE_DISABLE_BRAND' ) ) {
	...
	public static function hide_notices() {
		$hide_notice = filter_input( INPUT_GET, 'orderable-hide-notice', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	public static function output_getting_started_links() {
		...
		?>
		<h3><?php esc_html_e( 'Getting Started', 'orderable' ); ?></h3>
	...
	public static function setup_dashboard( $settings ) {
		...
		$settings['tabs'][] = array(
			'id'    => 'dashboard',
			'title' => esc_html__( 'Dashboard', 'orderable' ),
		);

		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) && ! defined( 'ORDERABLE_DISABLE_DASH' ) ) {
			$settings['sections']['license'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'general',
				'section_title'       => esc_html__( 'License &amp; Account Settings', 'orderable' ),
				'section_description' => '',
				'section_order'       => 10,
				'fields'              => array(
					array(
						'id'       => 'license_field',
						'title'    => __( 'License Key', 'orderable' ),
						'subtitle' => __( 'Activate your license to get the latest updates and support.', 'orderable' ),
						'type'     => 'custom',
						'output'   => '',
					),
					array(
						'id'       => 'account',
						'title'    => __( 'Your Account', 'orderable' ),
						'subtitle' => __( 'Manage all of your Iconic plugins, supscriptions, renewals, and more.', 'orderable' ),
						'type'     => 'custom',
			...
			$settings['sections']['support'] = array(
				'tab_id'              => 'dashboard',
				'section_id'          => 'support',
				'section_title'       => esc_html__( 'Support', 'orderable' ),
				'section_description' => '',
				'section_order'       => 30,
				'fields'              => array(
					array(
						'id'       => 'support',
						'title'    => esc_html__( 'Support', 'orderable' ),
						'subtitle' => esc_html__( 'Get premium support with a valid license.', 'orderable' ),
						'type'     => 'custom',
						'output'   => self::support_link(),
					),
					array(
						'id'       => 'documentation',
						'title'    => esc_html__( 'Documentation', 'orderable' ),
						'subtitle' => esc_html__( 'Read the plugin documentation.', 'orderable' ),
						'type'     => 'custom',
	...
	public static function add_sidebar() {
		...
		$cross_sells  = Orderable_Core_Cross_Sells::get_selected_plugins();
		$api_sidebars = Orderable_Core_Cross_Sells::get_api_sidebars();
		...
		<div class="iconic-settings-sidebar">
			...
			if ( ! empty( $cross_sells ) ) {
				...
				<div class="iconic-settings-sidebar__widget iconic-settings-sidebar__widget--works-well">
					<h3><?php esc_html_e( 'Works well with...', 'orderable' ); ?></h3>
									...
									<div class="iconic-product__buttons">
										<p>
											<a href="<?php echo esc_url( $checkout_url ); ?>" class="button iconic-button iconic-button--small" target="_blank">
												<?php esc_html_e( 'Buy Plugin', 'orderable' ); ?>
											</a>
										</p>
										<p><?php esc_html_e( '30 Day Money-Back Guarantee', 'orderable' ); ?></p>
									</div>
								<?php } ?>
							</div>
						</div>
	...
	public static function support_link() {
		...
		return sprintf(
			...
			esc_url($url),
			esc_html__( 'Submit Ticket', 'orderable' ) );
	}
	...
	public static function documentation_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			$url,
			esc_html__( 'Read Documentation', 'orderable' )
		);
	}
	...
	public static function account_link() {
		...
		return sprintf(
			'<a href="%s" class="button button-secondary" target="_blank">%s</a>',
			esc_url($url),
			esc_html__( 'Manage Your Account', 'orderable' )
		);
	}
	...
	public static function plugin_action_links( $links ) {
		...
		$action_links = array(
			'settings' => '<a href="' . admin_url( $settings_path ) . '" aria-label="' . esc_attr__( 'View settings', 'orderable' ) . '">' . esc_html__( 'Settings', 'orderable' ) . '</a>',
		);
	 ...
	 * @return string
	 */
	protected static function add_utm_query_args($url, $campaign, $medium = 'Plugin', $content = 'orderable', $source = null) {
		$args = [
